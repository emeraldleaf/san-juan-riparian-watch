...
This is a specialization of [Defer Await Until Needed](https://github.com/vercel-labs/agent-skills/blob/main/skills/react-best-practices/rules/async-defer-await.md) for `flag && cheapCondition` style checks.
...
For `await getFlag()` combined with a cheap synchronous guard (`flag && someCondition`), see [Check Cheap Conditions Before Async Flags](https://github.com/vercel-labs/agent-skills/blob/main/skills/react-best-practices/rules/async-cheap-condition-before-await.md).
...
For static assets and config, see [Hoist Static I/O to Module Level](https://github.com/vercel-labs/agent-skills/blob/main/skills/react-best-practices/rules/server-hoist-static-io.md).
