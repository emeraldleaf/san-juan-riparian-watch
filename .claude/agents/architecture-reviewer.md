...
riparian tests` from `python-etl`, then `ruff check experiments` from the **repo root** (its own
import tree — the root working dir is required for correct isort first-party detection). So
`experiments/**/*.py` gets the **same lint gate** as the package, and this review agent runs in
