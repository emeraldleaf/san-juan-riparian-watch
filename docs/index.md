...
| **[The corpus is a surface](corpus-automation.md)** | The project's own docs ARE the RAG corpus, so a stale index leaves the agent citing a claim every other surface has retracted. How re-ingest is mechanized, why retrieval is scoped into ours-vs-the-literature, and the rule that cost five blind production runs: look at the machine before changing it. |
