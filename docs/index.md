...
| **[▸ FM vs RF — the arroyo](fm-vs-rf-malpais.html)** | The held-out Malpais arroyo — the one morphology the RF had no training sibling for. The scored transfer is **RF 0.557 vs FM 0.889 AUROC** (leave-one-reach-out, shared label set) — that number is the evidence. ⚠️ The map's RF layer covers only part of the reach, so it is **not** a valid spatial comparison — see [the extent-artifact note](2026-08-10-arroyo-map-extent-artifact.md). |
