...
| extent-over-time-novel-unqualified | `Novel on its own` | `Perkins;automated;annual, automated` | **The claim that an extent time series is "novel on its own".** Too strong. **Perkins et al. (2025)** (*River Research and Applications*) mapped riparian **vegetated area** in Canyonlands NP from **1940 to 2022** — further back than our Landsat record reaches. Found 2026-07-12 by `/paper-audit`. It survives **only when qualified**: theirs is *aerial imagery, discrete dates, 152 km of one national park, area not species, no beetle*. Ours must say **annual, automated, wall-to-wall, satellite-derived — and species-level with defoliation as a state**. Say the qualified sentence, never the flat one. |
| arroyo-rf-fires-in-one-corner | `fires in one corner;fire.{0,4} in one corner` | `unequal extent;coverage boundary` | **The claim that the RF "fires in one corner" of the Malpais arroyo while the FM tracks the corridor, read from the map as model behavior.** The RF map layer (`rf_malpais`) spans only ~0.023×0.038° — a fifth of the width of the full-arroyo FM/truth layers — so the overlay compares **unequal extents** and confounds model behavior with export coverage; a 0.557-AUC field would be diffuse, not a tight blob (a **coverage boundary** is the likelier cause). Found 2026-08-10 by a reviewer asking "why one corner?". The **scored metric (RF 0.557 vs FM 0.889 AUROC)** is valid and **not** retracted; the spatial-visual narrative is. See [2026-08-10-arroyo-map-extent-artifact.md](2026-08-10-arroyo-map-extent-artifact.md). |
| arroyo-rescue-attribution-2026-08-21 | `desert arroyo;arroyo rescue;rescu[a-z]* .{0,15}arroyo` | `attribution to arroyo morphology is unverified;river-dominated subwatershed` | **The "desert arroyo rescue" — that OlmoEarth beat the RF (0.557 → 0.889) because the "Malpais" fold is a distinct DESERT ARROYO morphology the RF was blind to.** The "Malpais" reach is HUC12 140801051001, **"Malpais Arroyo–San Juan" — a river-dominated subwatershed**, not a desert arroyo. **Re-verification COMPLETE (2026-08-21):** the RF bar reproduced (Farmington 0.90 / Kirtland 0.85 / Aztec 0.89 / Malpais 0.56), and NMRipMap has **no labels up the wash**, so RF and FM were scored on the **same held-out pixels** — the comparison and the 0.557→0.889 number **STAND**. What is retracted is only the **arroyo attribution**: *attribution to arroyo morphology is unverified* — the reach is a San-Juan-valley subwatershed, and the mechanism is general **brittleness to distribution shift** (RF near-chance on native 0.59 AND invasive 0.53 alike — not arroyo shape, not invasive-specific). Do not restate "desert arroyo" / "arroyo rescue" as the *cause*; the transfer result itself is valid. Post-mortem: [reach-provenance gap](2026-08-21-reach-provenance-gap.md). |
<!-- RETRACTIONS:END -->
