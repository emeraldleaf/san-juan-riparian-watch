...
| 15 | `rslearn dataset materialize` **exited 0** | …having written **zero** files. `NotImplementedError` on all 238 windows, swallowed into a worker pool. `"ingest": false` needs `get_item_by_name`, which Planetary Computer's `Sentinel2` **raises by design** | `verify_materialized()` — check the rasters are on disk, never the exit code |
| 16 | The plan's storage estimate, **"~1.2 GB"** | Right for the *materialised* chips, **wrong by ~10× for `ingest`** (11 GB tile store), which pulls whole 110 km granules. And it staged through `TMPDIR` on the **boot disk** — filling `/` to zero, hard enough that no tool could write, *including the tools needed to clean up* | pinning `TMPDIR` to the data drive; budgeting the tile store, not the output |
| 17 | `TMPDIR` on the data drive — and materialize **still** wrote 2.8 GB to `/` | **GDAL keeps its own temp**, `CPL_TMPDIR`, which `TMPDIR` does not cover. Fixing one leak hid the second: same symptom, different env var | redirecting *every* temp mechanism, not the first one that shows up |
| 18 | `model.yaml` said **`num_classes: 4`** | The crosswalk emits **four** real classes and `zero_is_invalid` reserves class 0, so it needs **5**. Authored for an older 3-class scheme; the split-out of agriculture never propagated. Crashed `Target 4 is out of bounds` at sanity-check | `tests/test_class_scheme_contract.py` — num_classes == max crosswalk id + 1, mechanically |
| 19 | `SegmentationPoolingDecoder` on a **temporal** cube | Reads spatial dims as `image.shape[1:3]`; our input is 4-D `[bands, timesteps, H, W]`, so it grabbed `(timesteps, H)` and predicted `12×2` against a `2×2` target. The whole approach *is* the 12-mosaic time axis, so this is not a corner case | a `TemporalSegmentationPoolingDecoder` adapter reading the true last-two axes; found by a laptop dry-run, not a GPU |
| 20 | A cheap **single-scene** S2 fetch, to skip re-materializing a second reach | Farmington→Malpais transfer collapsed to **AUC 0.37 — worse than random.** Not a transfer failure: single scenes vs 30-day median mosaics sample *different phenological moments*, so a phenology-driven model saw out-of-distribution features | re-materializing Malpais on the **identical** 12-month median compositing (0.37 → **0.80 AUC**). Aligned compositing *is* the experiment, not a preprocessing detail — now canon (`materialize_reach.py`, CLAUDE.md) |
| 21 | The merge gate itself, a **third** time — `check-coderabbit.sh` called `gh api --jq --arg h "$HEAD"` | **`gh api`'s `--jq` does not accept jq's `--arg`** — gh errors (`accepts 1 arg(s), received 4`), so `REVIEW` came back **empty** and the gate reported "unaddressed finding" on *every* approved-with-findings PR. It **false-negatived #61** — which CodeRabbit had **APPROVED on the head** — and stalled the merge poller that relied on it | trusting GitHub's own `reviewDecision=APPROVED` over the wrapper that disagreed with the API it claimed to read; fixed by piping to real `jq` (which *does* take `--arg`). The lesson from #13 repeats: **a broken gate reports a verdict it did not compute** |
| 22 | The cross-sensor fetch wrote **empty months** for several S2 dates | **A transient STAC search timeout was indistinguishable from genuine no-data** — both became an all-NaN month, and the Phase-3A run *silently* trained on a poisoned cube. A later variant even **saved the cube to cache before raising**, so `--score-only` would reuse the poison | the fetch now flags `SEARCH-FAILED` months separately from genuine-empty, **refuses to persist a cube with any transient failure**, and allocates a fresh read buffer per retry. Verified by rerunning to a clean 12/12 (`phase3a_cross_sensor.py`, `phase3b_temporal.py`) — CodeRabbit caught the save-before-raise on #66 |
