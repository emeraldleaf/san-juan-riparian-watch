...
(AUROC < 0.5) when pushed back before the tamarisk beetle — is **falsified**: tamarisk-vs-native stays
**0.85–0.87 across 2020 → 2015 → 2000**, never near a flip. And the pre-registered **negative control
(Russian olive) dropped 0.285** over the same span — so even the faint signal that *is* in the expected
direction sits well inside the noise this data carries. **The discriminator is beetle-robust, and this
in-basin sample is too small and too cross-sensor-confounded to resolve a subtle beetle effect anyway.**
...
**not** move. Ground truth: **167 CSU 2017 field points** in the San Juan basin (47 tamarisk, 49 Russian
olive, 39 native). Leave-one-**spatial-block**-out CV (~2 km tiles) — the in-basin points are effectively
one field cluster (SW Colorado), so leave-one-*trip*-out is impossible (a trip can be single-class).

## The result

| taxon-vs-native | 2020 (Test A) | 2015 post-beetle | 2000 **pre-beetle** | Δ(2000−2020) |
|---|---|---|---|---|
| **tamarisk** (signal) | 0.853 | 0.815 | **0.868** | **+0.014** |
| **Russian olive** (control) | 0.879 | 0.791 | **0.594** | **−0.285** |

Two readings, and the second is the important one:

1. **No inversion.** A 2020-trained model still ranks tamarisk-vs-native at **0.868 in 2000** — nowhere
   near the predicted < 0.5. There is a *faint* move in the predicted direction (post-beetle 0.815 <
   pre-beetle 0.868, consistent with the beetle removing the late-season cue), but it is ~0.05.
2. **The control vetoes it.** Russian olive — which the beetle does not touch — **cratered 0.285**
   pre-beetle. A control that should be flat instead moved 5× more than the tamarisk "signal." So the
   ...
   sample variance. That ~0.28 noise floor dwarfs the ~0.05 tamarisk effect.
...
it — which is what 0.868 → 0.815 (a dip, not a flip) looks like.
...
2. **The sensor confound is real and unquantified here.** 2000 = Landsat-5 TM, 2020 = Landsat-8 OLI; the
   control's −0.285 is the clearest evidence that TM↔OLI + small-n drift, not the beetle, drives the
