...
└── scripts/
    └── vendor_harness.sh   # copies the trimmed harness from the prior project repo
```
...
`quartzose/production-app/app/backend` (ADR: copy over submodule/subtree — simplest to run
+ deploy; upstream is stable). Run:

```bash
bash docintel/scripts/vendor_harness.sh            # uses the default prior project path
