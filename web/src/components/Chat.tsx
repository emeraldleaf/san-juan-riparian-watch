...
const SUGGEST = [
  ...
  'How does OlmoEarth compare to the Random Forest — and what result is under review?',
...
const FALLBACK: { k: string[]; a: string }[] = [
  ...
  { k: ['foundation', 'fm', 'beat', 'rf', 'random forest', 'tie', 'arroyo', 'malpais', 'olmoearth'], a: "On the river reaches, RF and OlmoEarth broadly tie at ~0.80–0.88 transfer AUC. The headline “arroyo rescue” — RF 0.557 → OlmoEarth 0.889 on the “Malpais” fold — is UNDER REVIEW and currently retracted. That reach is the mixed “Malpais Arroyo–San Juan” subwatershed, dominated by the river; on the maps the models sit on the San Juan while the ground truth follows the wash. the LORO scored only the imaged San Juan-valley windows, and the defined reach's northern arroyo was never imaged or scored — so the open question is narrower: whether the 0.557→0.889 gap comes from the arroyo or the river portion of the mixed reach. Attribution to arroyo morphology is unverified, so we're re-auditing the scoring before restating anything." },
...
function fallbackAnswer(text: string): string {
  ...
  return bs > 0 ? (best as string) : "Offline right now, I can speak to what's on this page: the 23% invasive share, the RF-vs-OlmoEarth transfer test (and the arroyo result now under review), the beetle control, the pre-2000 negative, or why NDVI isn't enough.";
}
