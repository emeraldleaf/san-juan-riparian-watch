...
if (container && !webglSupported()) mapFallback(container);
  ...
  const PRESENTATION: { title: string; scenes: Scene[] } = {
    ...
    scenes: [
      { narration: "A reach on the San Juan River, in New Mexico's high desert. A thin ribbon of riparian vegetation follows the water through otherwise dry country. The job: map it from space, then tell native from invasive.", layers: [], fit: 'truth', pitch: 35 },
      ...
      { narration: "The Random Forest learns the seasonal fingerprint and predicts riparian, shown in green over the grey truth. It does well on the river corridors it was trained across.", layers: ['btruth', 'brf'], fit: 'brf' },
